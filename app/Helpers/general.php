...
if (!function_exists('get_country_list')) {
    function get_country_list($old_data = '')
    {
        if ($old_data == '') {
            ...
            $country_list = file_get_contents(
                app_path() . '/Helpers/country.txt'
            );
...
if (!function_exists('show_price')) {
    function show_price($amount, $show_symbol = true)
    {
        ...
        if ($base_rate == '') {
            $base_rate = \App\Currency::where('name', currency())->first()
                ->exchange_rate;
            \Cache::put('base_rate', $base_rate);
        }

        if ($display_currency_rate == '') {
            $display_currency_rate = \App\Currency::where(
                'name',
                $currency
            )->first()->exchange_rate;
        ...
        if ($show_symbol == true) {
            if (get_currency_position() == 'right') {
                return money_format_2($amount) .
                    ' ' .
                    get_currency_symbol($currency);
            } else {
                return get_currency_symbol($currency) .
                    ' ' .
                    money_format_2($amount);
            }
        }
...
if (!function_exists('permission_list')) {
    ...
    {
        $permission_list = \App\AccessControl::where(
            'role_id',
            Auth::user()->role_id
        )
            ->pluck('permission')
...
if (!function_exists('get_currency_list')) {
    ...
    {
        $currency_list = file_get_contents(
            app_path() . '/Helpers/currency.txt'
        );
        ...
        if ($old_data == '') {
            ...
            if ($serialize == true) {
                ...
                $pattern = '<option value="' . $old_data . '">';
                $replace =
                    '<option value="' . $old_data . '" selected="selected">';
                $currency_list = str_replace(
                    $pattern,
                    $replace,
                    $currency_list
...
if (!function_exists('file_icon')) {
    function file_icon($mime_type)
    {
        ...
        return isset($font_awesome_file_icon_classes[$mime_group])
            ? $font_awesome_file_icon_classes[$mime_group]
            : 'fa-file';
    }
}
        ...
        if ($last_run >= 12 || $force == true) {
            ...
            foreach ($currency_list as $currency) {
                if (isset($exchangeRates['rates'][$currency->name])) {
                    ...
                    $rate =
                        $exchangeRates['rates'][$currency->name] /
                        $exchangeRates['rates'][$system_base_currency];
...
if (!function_exists('convert_currency')) {
    ...
    {
        $currency1 = \App\Currency::where('name', $from_currency)->first()
            ->exchange_rate;
        $currency2 = \App\Currency::where('name', $to_currency)->first()
            ->exchange_rate;
...
if (!function_exists('convert_currency_2')) {
    ...
    {
        $currency1 = (int) $currency1_rate;
        $currency2 = (int) $currency2_rate;
...
if (!function_exists('get_variation_price')) {
    ...
    {
        $variationPrices = \App\Entity\Product\ProductVariationPrice::where(
            'product_id',
            $product_id
        )->get();
        ...
        foreach ($variationPrices as $variation_price) {
            ...
            if (empty($result)) {
                $variation_items = \App\Entity\Product\ProductVariationItem::whereIn(
                    'id',
                    $product_options
                )->get();
                ...
                if ($symbol == true) {
                    ...
                    $special_price =
                        $variation_price->special_price != ''
                            ? show_price($variation_price->special_price)
                            : '';
                    break;
                } elseif ($symbol == false) {
...
if (!function_exists('get_all_country')) {
    ...
    {
        $data = json_decode(
            file_get_contents(app_path() . '/Helpers/countries.json')
        );
...
if (!function_exists('get_states')) {
    ...
    {
        $data = json_decode(
            file_get_contents(app_path() . '/Helpers/states.json')
        );
        ...
        foreach ($object as $menu) {
            if ($currentParent == $menu->parent_id) {
                ...
                echo '<li class="dd-item" data-id="' .
                    ...
                    '</div><a class="edit_menu" href="' .
                    action("$controller@edit", $menu->id) .
                    '"><i class="far fa-edit"></i></a>
					<a class="btn-remove-2 remove_menu" href="' .
                    action("$controller@destroy", $menu->id) .
    ...
    function buildTable(
        ...
        foreach ($object as $category) {
            if ($currentParent == $category->parent_id) {
                ...
                echo '<td>' .
                    '<form action="' .
                    route($url . '.destroy', $category->id) .
                    '" method="post">
				<a href="' .
                    route($url . '.edit', $category->id) .
                    '" class="btn btn-warning btn-xs"><i class="fa fa-pencil" aria-hidden="true"></i></a>
				' .
                    method_field('DELETE') .
                    '
				' .
    ...
    function buildTree(
        ...
        foreach ($object as $category) {
            if ($currentParent == $category->parent_id) {
                ...
                echo '<li> <label class="menu_label" for=' .
                    $category->id .
                    '>
				<a href="' .
                    route($url . '.edit', $category->id) .
            ...
            if ($currentParent == $category->parent_id) {
                if ($currLevel > $prevLevel) {
                    if ($category->parent_id != null) {
                        echo "<ul class='categor-list collapse' id='category-" .
                            $category->parent_id .
                            "'>";
                    } else {
                ...
                if ($category->child_category->count() > 0) {
                    echo '<li>
					<i class="fa fa-angle-right" role="button" aria-hidden="true" data-toggle="collapse" data-target="#category-' .
                        $category->id .
    ...
    function buildNavigation(
        ...
        foreach ($navigation_items as $nav_item) {
            if ($currentParent == $nav_item->parent_id) {
                ...
                } elseif ($currLevel > $prevLevel && $currLevel == 1) {
                    echo "<ul class='$dropdown_1_class'>";
                } elseif ($currLevel > $prevLevel && $currLevel == 2) {
                ...
                } elseif ($nav_item->type == 'page') {
                    $url = url('/' . $nav_item->page->slug);
                } elseif ($nav_item->type == 'category') {
                    $url = url('/categories/' . $nav_item->category->slug);
                } elseif ($nav_item->type == 'custom_url') {
                    $url = $nav_item->url;
                }
                ...
                $css_class =
                    $nav_item->css_class != ''
                        ? "class='$nav_item->css_class'"
                        : '';
                $css_id =
                    $nav_item->css_id != '' ? "id='$nav_item->css_id'" : '';
...
if (!function_exists('show_admin_comment')) {
    function show_admin_comment($comment)
    {
        ...
        echo '<li><span class="menu_label">' .
            $comment->body .
            '<a href="' .
            route('product_comments.destroy', $comment->id) .
            '" class="text-danger float-right btn-remove-2"><i class="fas fa-eraser"></i></a></span>';
...
if (!function_exists('get_max_price')) {
    ...
    {
        $general_product_price = \App\Entity\Product\Product::where(
            'is_active',
            1
        )->max('price');
        $variable_product_price = \App\Entity\Product\ProductVariationPrice::max(
            'price'
        );
