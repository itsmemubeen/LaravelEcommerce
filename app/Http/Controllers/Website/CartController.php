...
class CartController extends Controller
{
    ...
    public function add_to_cart(Request $request, $product_id)
    {
        ...
        if ($product->product_type != 'variable_product') {
            ...
            if ($product->manage_stock == 1) {
                ...
                if ($item_quantity + $request->quantity > $product->qty) {
                    return response()->json([
                        'result' => false,
                        'message' => _lang('Sorry, No stock available !'),
                    ]);
                }
            }
            ...
            if (!$request->ajax()) {
                ...
                $view = view(
                    "theme.$this->theme.components.mini-cart"
                )->render();
            ...
            $index = 0;
            foreach (
                explode('&', $request->product_option[0])
                as $product_option
            ) {
    ...
    public function update_cart(Request $request)
    {
        ...
        if (!$request->ajax()) {
            ...
            $shopping_cart = view(
                "theme.$this->theme.components.cart-body"
            )->render();
            $mini_cart = view(
                "theme.$this->theme.components.mini-cart"
            )->render();
    ...
    public function apply_coupon(Request $request)
    {
        ...
        if ($coupon) {
            ...
            if ($coupon->usage_limit_per_coupon == $coupon->used) {
                return back()->with(
                    'error',
                    _lang('Coupon already reached the maximum number of used !')
                );
            }
            ...
            if ($coupon->usage_limit_per_customer > 0) {
                if (!Auth::check()) {
                    return back()->with(
                        'error',
                        _lang(
                            'Sorry, This Coupon code has usages restriction. Please login before apply this coupon code !'
                        )
                    );
                }
                ...
                if (
                    $coupon->usage_limit_per_customer == $customer_usages_limit
                ) {
                    return back()->with(
                        'error',
                        _lang(
                            'Sorry, You have already reached the maximum number of used !'
                        )
                    );
                }
            }
            ...
            if ($coupon->minimum_spend > 0) {
                if (Cart::getSubTotal() < $coupon->minimum_spend) {
                    return back()->with(
                        'error',
                        _lang('Minimum Spend Amount') .
                            ' ' .
                            show_price($coupon->minimum_spend)
                    );
                }
            }
            ...
            if ($coupon->maximum_spend > 0) {
                if (Cart::getSubTotal() > $coupon->maximum_spend) {
                    return back()->with(
                        'error',
                        _lang('Sorry, You have Spend maximum amount !')
                    );
                }
            }
            ...
            if (!$coupon_products->isEmpty()) {
                ...
                foreach (Cart::getContent() as $cart) {
                    $row = $coupon_products->firstWhere(
                        'id',
                        $cart->associatedModel->id
                    );
            ...
            if (!$exclude_products->isEmpty()) {
                ...
                foreach (Cart::getContent() as $cart) {
                    $row = $exclude_products->firstWhere(
                        'id',
                        $cart->associatedModel->id
                    );
            ...
            if (!$coupon_categories->isEmpty()) {
                ...
                foreach (Cart::getContent() as $cart) {
                    $row = $coupon_categories
                        ->whereIn(
                            'id',
                            $cart->associatedModel->categories->pluck('id')
                        )
                        ->all();
            ...
            if ($coupon_amount > 0) {
                $coupon_value =
                    $coupon->is_percent == 1
                        ? ($coupon->value / 100) * $coupon_amount
                        : $coupon->value;
            } elseif ($general_coupon == true) {
                $coupon_value =
                    $coupon->is_percent == 1
                        ? ($coupon->value / 100) * Cart::getSubTotal()
                        : $coupon->value;
            }
            ...
            if ($coupon_value > 0) {
                ...
                $coupon001 = new \Darryldecode\Cart\CartCondition([
                ...
                return back()->with(
                    'success',
                    _lang('Coupon Applied Sucessfully')
                );
            } else {
                return back()->with(
                    'error',
                    _lang('You are not eligible for this coupon code !')
                );
            }
        } else {
