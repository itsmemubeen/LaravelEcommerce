...
class CustomerController extends Controller
{
    ...
    public function sign_in(Request $request)
    {
        ...
        if ($request->isMethod('get')) {
            ...
            if (
                ...
                if (
                    get_option('email_verification') == 'enabled' &&
                ...
                return back()
                    ->with(
                        'error',
                        _lang('Username or password is incorrect !')
                    )
                    ->withInput();
            }
        }
    ...
    public function sign_up(Request $request)
    {
        ...
        if ($request->isMethod('get')) {
            ...
            $user->email_verified_at =
                get_option('email_verification') == 'disabled' ? now() : null;
            ...
            if (!$request->ajax()) {
                return redirect('/sign_in')->with(
                    'success',
                    _lang('Your account has been created sucessfully')
                );
            } else {
                return response()->json([
                    'result' => 'success',
    ...
    public function my_account(Request $request, $page = 'overview')
    {
        ...
        if ($page == 'overview') {
            ...
            $data['orders'] = Order::where('customer_id', Auth::id())->paginate(
                10
            );
            return view("theme.$this->theme.customer.my_account.$page", $data);
            ...
            $data['downloads'] = \App\OrderProduct::whereHas(
                'product',
            ...
            $data['addresses'] = CustomerAddress::where(
                'customer_id',
                Auth::id()
            )->get();
            return view("theme.$this->theme.customer.my_account.$page", $data);
        } elseif ($page == 'account_details') {
            return view("theme.$this->theme.customer.my_account.$page", $data);
    ...
    public function download_product($order_product_id)
    {
        ...
        $orderProduct = \App\OrderProduct::whereHas('product', function (
            Builder $query
        ) {
            $query->where('product_type', 'digital_product');
        })
    ...
    public function order_details(Request $request, $order_id)
    {
        ...
        $order = Order::where('id', $order_id)
            ...
            ->first();
        return view(
            "theme.$this->theme.customer.my_account.order-details",
            compact('order')
        );
    }
    ...
    public function update_account(Request $request)
    {
        ...
        if (!$request->ajax()) {
            ...
            return response()->json([
                'result' => 'success',
                'action' => 'store',
                'message' => _lang('Updated sucessfully'),
                'data' => $user,
                'table' => '#users_table',
            ]);
        }
    ...
    public function update_password(Request $request)
    {
        ...
        if (!$request->ajax()) {
            return back()->with(
                'success',
                _lang('Password Updated sucessfully')
            );
        } else {
            return response()->json([
                'result' => 'success',
                'action' => 'store',
                'message' => _lang('Password Updated sucessfully'),
                'data' => $user,
                'table' => '#users_table',
            ]);
        }
    ...
    public function add_new_address(Request $request)
    {
        if ($request->isMethod('get')) {
            return view(
                "theme.$this->theme.customer.my_account.create_address",
                ['page' => 'addresses']
            );
        }
        ...
        if (!$request->ajax()) {
            return redirect('my_account/addresses')->with(
                'success',
                _lang('New address added successfully')
            );
        } else {
            return response()->json([
                'result' => 'success',
                'action' => 'store',
                'message' => _lang('New address added successfully'),
                'data' => $customeraddress,
                'table' => '#customer_addresses_table',
            ]);
        }
    }
    ...
    public function update_address(Request $request, $address_id)
    {
        if ($request->isMethod('get')) {
            ...
            $page = 'addresses';
            return view(
                "theme.$this->theme.customer.my_account.edit_address",
                compact('address', 'page')
            );
        }
    ...
    {
        $customeraddress = CustomerAddress::where('id', $address_id)->where(
            'customer_id',
            Auth::id()
        );
        ...
        return redirect('my_account/addresses')->with(
            'success',
            _lang('Address Removed Successfully')
        );
    }
