...
class GatewayController extends Controller
{
    ...
    public function confirm_order($type, $order_id)
    {
        ...
        if ($order->transaction()->exists()) {
            return redirect('/')->with(
                'error',
                _lang('Sorry, Payment has already made for this order !')
            );
        }
        ...
        if ($order->status == $order::PENDING_PAYMENT) {
            ...
            $order->payment_method =
                $type == 'cod' ? 'Cash_On_Delivery' : 'Bank_Transfer';
            $order->save();
    ...
    public function paypal_payment_authorize($paypalOrderId, $order_id)
    {
        ...
        try {
            ...
            if ($response->result->status == 'COMPLETED') {
                ...
                return view(
                    "theme.$this->theme.order-complete",
                    compact('order')
                );
            }
            ...
            return back()->with(
                'error',
                _lang(
                    'Sorry, Payment not completed, Please contact with your administrator !'
                )
            );
        }
    }
    ...
    public function razorpay_payment_authorize(Request $request, $order_id)
    {
        ...
        try {
            ...
            return back()->with(
                'error',
                _lang('Sorry, Payment not completed !')
            );
        }
    }

    public function paystack_payment_authorize(
        Request $request,
        $order_id,
        $reference
    ) {
