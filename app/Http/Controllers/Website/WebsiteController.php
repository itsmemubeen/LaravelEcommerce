...
class WebsiteController extends Controller
{
    ...
    {
        $this->theme = env('ACTIVE_THEME', 'newtheme');
    ...
    public function index($slug = '')
    {
        ...
        if ($page->template != null) {
            return view(
                "theme.$this->theme.templates.template-$page->template",
                compact(
                    'page',
                    'seo_title',
                    'meta_keywords',
        ...
        return view(
            "theme.$this->theme.page",
            compact('page', 'seo_title', 'meta_keywords', 'meta_description')
        );
    }
    ...
    public function product($slug = '')
    {
        ...
        if (request()->ajax()) {
            $productView = view(
                "theme.$this->theme.components.quick_shop",
                compact('product')
            )->render();
        ...
        return view(
            "theme.$this->theme.single-product",
            compact('product', 'seo_title', 'meta_keywords', 'meta_description')
        );
    }

    public function single_cooler()
    {
        $productone = Product::where('slug', 'essential-cooler-1')
            ->where('is_active', 1)
            ->first();

        $producttwo = Product::where('slug', 'essential-cooler-5')
            ->where('is_active', 1)
            ->first();

        $tagsone = DB::table('tags_translation')
            ->select('tags_translation.name as name')
            ->join('tags', 'tags_translation.tag_id', '=', 'tags.id')
            ->join('product_tags', 'tags.id', '=', 'product_tags.tag_id')
            ->where('product_tags.product_id', '=', 1)
            ->get();

        $tagstwo = DB::table('tags_translation')
            ->select('tags_translation.name as name')
            ->join('tags', 'tags_translation.tag_id', '=', 'tags.id')
            ->join('product_tags', 'tags.id', '=', 'product_tags.tag_id')
            ->where('product_tags.product_id', '=', 6)
            ->get();
        ...
        return view(
            "theme.$this->theme.single-cooler",
            compact('productone', 'producttwo', 'tagsone', 'tagstwo')
        );
    }
    ...
    {
        $category = \App\Entity\Category\Category::where(
            'slug',
            $slug
        )->first();
        ...
        $seo_title =
            $category->translation->name . ' - ' . get_option('site_title');

        return view(
            "theme.$this->theme.shop",
            compact('products', 'slug', 'seo_title')
        );
    }
    ...
    public function brands($slug = '')
    {
        if ($slug == '') {
            ...
            return view(
                "theme.$this->theme.brands",
                compact('brands', 'seo_title')
            );
        } else {
            $brand = \App\Entity\Brand\Brand::where('slug', $slug)->first();
            $products = $this->filter($brand->products());
            $seo_title =
                $brand->translation->name . ' - ' . get_option('site_title');
        }

        $brand_slug = $slug;
        return view(
            "theme.$this->theme.shop",
            compact('products', 'brand_slug', 'seo_title')
        );
    }
    ...
    public function tags($slug = '')
    {
        ...
        return view(
            "theme.$this->theme.shop",
            compact('products', 'tag_slug', 'seo_title')
        );
    }
    ...
    public function wish_list(Request $request, $product_id = '')
    {
        if (!Auth::check()) {
            if ($request->ajax()) {
                return response()->json([
                    'result' => false,
                    'message' => _lang('You need to login first !'),
                ]);
            } else {
            ...
            if ($request->ajax()) {
                return response()->json([
                    ...
                    'total_items' => auth()
                        ->user()
                        ->wishlist->count(),
                    'message' => _lang('Added to Wishlist'),
                ]);
            } else {
                return back();
            }
                ...
                $shipping = new \Darryldecode\Cart\CartCondition([
                    'name' =>
                        get_option('flat_rate_active') == 'Yes'
                            ? get_option('flat_rate_label')
                            : get_option('local_pickup_label'),
                    'type' => 'shipping',
                    'target' => 'total',
                    'value' =>
                        get_option('flat_rate_active') == 'Yes'
                            ? '+' . get_option('flat_rate_cost')
                            : '+' . get_option('local_pickup_cost'),
                    'order' => 1,
                ]);
                Cart::condition($shipping);
            }
    ...
    public function checkout()
    {
        ...
        if (
            ...
            return redirect('/')->with(
                'checkout_error',
                _lang('No Shipping Method found !')
            );
        }
        ...
        if (count($shipping) == 0 && $cartTotal > 0) {
            if (
                get_option('free_shipping_active') == 'Yes' &&
                $cartTotal >= get_option('free_shipping_minimum_amount')
            ) {
                $shipping = new \Darryldecode\Cart\CartCondition([
                    'name' => get_option('free_shipping_label'),
                    'type' => 'shipping',
    ...
    public function send_message(Request $request)
    {
        ...
        if (get_option('email') != '') {
            try {
                ...
                return back()->with(
                    'success',
                    _lang('Your Message send sucessfully.')
                );
            } catch (\Exception $e) {
                return back()->with(
                    'error',
                    _lang('Error Occured, Please try again !')
                );
            }
        }
    }
    ...
    public function subscribe_newsletter(Request $request)
    {
        ...
        return back()->with(
            'error',
            _lang('Sorry, You may already subscribed !')
        );
    }
    ...
    private function filter($products)
    {
        ...
        $length = isset(request()->length) ? request()->length : 9;
        $sort_by = isset(request()->sort_by) ? request()->sort_by : '';
        $start_price = isset(request()->start_price)
            ? request()->start_price
            : 0;
        $end_price = isset(request()->end_price)
            ? request()->end_price
            : get_max_price();
        ...
        $filter_products = $products
            ...
            ->whereRaw(
                "IF(special_price != null, special_price, price) between $start_price AND $end_price"
            )
            ->paginate($length)
            ->withQueryString();
        ...
        if ($search != '') {
            if ($category == '' || $category == 'all') {
                ...
                $result = Product::join(
                    ...
                    ->whereHas('categories', function (Builder $query) use (
                        $category
                    ) {
    ...
    {
        return view('theme.newtheme.aboutus');
    }
}