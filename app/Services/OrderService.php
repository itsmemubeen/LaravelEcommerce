    ...
    private function store($request)
    {
        ...
        foreach ($cartConditions as $condition) {
            if ($condition->getType() == 'shipping') {
                ...
                $discount = str_replace(['+', '-'], '', $condition->getValue());
            }
        }
        ...
        return Order::create([
            'customer_id' => auth()->id(),
            'customer_name' =>
                $auth == false ? $request->name : Auth::user()->name,
            'customer_email' =>
                $auth == false ? $request->email : Auth::user()->email,
            'customer_phone' =>
                $auth == false ? $request->phone : Auth::user()->phone,

            'billing_name' =>
                $auth == false ? $request->name : $shipping_address->name,
            'billing_city' =>
                $auth == false ? $request->city : $shipping_address->city,
            'billing_state' =>
                $auth == false ? $request->state : $shipping_address->state,
            'billing_post_code' =>
                $auth == false
                    ? $request->post_code
                    : $shipping_address->post_code,
            'billing_country' =>
                $auth == false ? $request->country : $shipping_address->country,
            'billing_address' =>
                $auth == false ? $request->address : $shipping_address->address,

            'shipping_name' =>
                $auth == false ? $request->name : $billing_address->name,
            'shipping_city' =>
                $auth == false ? $request->city : $billing_address->city,
            'shipping_state' =>
                $auth == false ? $request->state : $billing_address->state,
            'shipping_post_code' =>
                $auth == false
                    ? $request->post_code
                    : $billing_address->post_code,
            'shipping_country' =>
                $auth == false ? $request->country : $billing_address->country,
            'shipping_address' =>
                $auth == false ? $request->address : $billing_address->address,
